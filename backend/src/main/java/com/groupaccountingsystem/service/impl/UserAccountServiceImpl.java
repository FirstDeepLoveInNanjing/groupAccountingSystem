...
public class UserAccountServiceImpl implements UserAccountService {
    ...
    @Override
    public OrdinaryUser getUserInfoWithBill(Integer userID) {
        OrdinaryUser user = userMapper.selectById(userID);
        if (user != null) {
            user.setTotalIncome(payMapper.sumPaidToActivityCreator(userID));
            user.setTotalExpense(payMapper.sumPaidByUserID(userID));
        }
        return user;
    }
    ...
    @Override
    public List<OrdinaryUser> listUsers(String keyword) {
        return userMapper.searchUsers(keyword);
    }
