...
public class MainActivity extends Activity {
        ...
        ApiClient.get().warnings().enqueue(new Callback<ApiResponse<List<Report>>>() {
            ...
            public void onResponse(Call<ApiResponse<List<Report>>> call, Response<ApiResponse<List<Report>>> response) {
                ...
                if (body != null && body.isSuccess() && body.data != null && !body.data.isEmpty()) {
                    addSectionTitle("⚠️ 警告举报");
                    for (Report report : body.data) {
                        addCardText("举报#" + report.reportID
    ...
    private void addActivityFilterControls() {
        addCardText("筛选：关键词=" + safe(activityKeyword)
                + "\n结算状态=" + settleFilterText(activityIsSettable)
                + "  我创建的未结算=" + yesNo(activityMyCreatedUnsettled));
        addSecondaryButton("筛选/搜索活动", v -> showActivityFilterDialog());
    ...
    private void renderActivityDetail(ActivityDetail detail) {
        ...
        addSectionTitle(activity.titleLine());
        if (activity.description != null && !activity.description.isEmpty()) {
            addCardText("说明：" + activity.description);
        }
        addCardText("实际金额：" + activity.actualAmount
                + "\n创建者：" + safe(activity.creatorName) + "  成员数：" + (detail.members == null ? 0 : detail.members.size())
                + "\n确认已发起：" + yesNo(activity.confirmInitiated)
                + "\n我的确认状态：" + safe(activity.currentUserConfirmStatus)
                + "\n结算状态：" + yesNo(activity.isSettable)
                + (activity.settleUnavailableReason != null && !activity.settleUnavailableReason.isEmpty()
                        ? "\n不可结算原因：" + activity.settleUnavailableReason : ""));

        addSectionTitle("成员");
        if (detail.members == null || detail.members.isEmpty()) {
            addSecondaryText("暂无成员");
        } else {
            for (ActivityMember member : detail.members) {
                addCardText("#" + member.memberID + " " + safe(member.memberName)
                        + "\n角色：" + safe(member.role)
                        + "  确认状态：" + safe(member.confirmAttendStatus));
                if (Boolean.TRUE.equals(detail.isCreator) && !ROLE_CREATOR.equals(member.role)) {
                    if (STATUS_QUIT_APPLY.equals(member.confirmAttendStatus)) {
                        addSecondaryButton("同意退出 #" + member.memberID,
                        ...
                        addDangerButton("拒绝退出 #" + member.memberID,
                    ...
                    addDangerButton("移除成员 #" + member.memberID,
        ...
        if (detail.collect != null) {
            addSectionTitle("收款信息");
            addCardText("金额：" + detail.collect.amount + "  状态：" + safe(detail.collect.collectStatus)
                    + "\n说明：" + safe(detail.collect.description));
        }
        if (detail.pays != null && !detail.pays.isEmpty()) {
            addSectionTitle("付款记录");
            for (Pay pay : detail.pays) {
                addCardText("付款#" + pay.payID + " 用户#" + pay.userID
                        + "\n金额：" + pay.payAmount + " 状态：" + safe(pay.payStatus));
            }
        }

        if (Boolean.TRUE.equals(detail.isCreator)) {
            addSectionTitle("创建者操作");
            if (!Boolean.TRUE.equals(activity.isSettable)) {
                addButton("编辑项目", v -> showEditActivityDialog(activity));
                addDangerButton("删除未结算项目", v -> confirmDeleteActivity(activity.activityID));
                addSecondaryButton("添加成员", v -> showAddMemberDialog(activity.activityID));
            }
            ...
            if (Boolean.TRUE.equals(activity.canSettle) && !Boolean.TRUE.equals(activity.isSettable)) {
                ...
                addSecondaryText("暂不可结算：" + safe(activity.settleUnavailableReason));
            }
        } else {
            addSectionTitle("参与者操作");
            if (Boolean.TRUE.equals(activity.confirmInitiated)
                ...
                addButton("确认参加", v -> simpleAction(ApiClient.get().confirm(activity.activityID), "已确认",
                        () -> loadActivityDetail(activity.activityID)));
                addSecondaryButton("申请退出", v -> simpleAction(ApiClient.get().applyQuit(activity.activityID), "已提交退出申请",
                        () -> loadActivityDetail(activity.activityID)));
                ...
                addCardText("⏳ 退出申请待创建者处理");
            } else if (!Boolean.TRUE.equals(activity.confirmInitiated)) {
                addSecondaryText("创建者尚未发起项目确认");
            }
        }
        addDangerButton("发布举报", v -> showReportDialog(activity.activityID));
        addSecondaryButton("返回活动列表", v -> loadActivities());
    }
    ...
    private void loadPendingSettlements() {
        ...
        ApiClient.get().myPays().enqueue(new Callback<ApiResponse<List<Pay>>>() {
            ...
            public void onResponse(Call<ApiResponse<List<Pay>>> call, Response<ApiResponse<List<Pay>>> response) {
                ...
                if (unpaidCount == 0) {
                    addSecondaryText("暂无待支付记录");
                }
                addSectionTitle("待收款");
                addCardText("当前 Android 端缺少后端个人收款统计接口，待收款仍需从活动详情的付款记录查看。");
            }
    ...
    private void loadPaymentDetail(Integer payID) {
        ...
        ApiClient.get().payDetail(payID).enqueue(new Callback<ApiResponse<PaymentDetail>>() {
            ...
            public void onResponse(Call<ApiResponse<PaymentDetail>> call, Response<ApiResponse<PaymentDetail>> response) {
                ...
                Pay pay = detail.pay;
                addSectionTitle("付款#" + pay.payID + " 活动#" + pay.activityID);
                if (detail.activity != null) {
                    addCardText("活动：" + detail.activity.titleLine()
                            + "\n金额：" + pay.payAmount + " 状态：" + safe(pay.payStatus)
                            + (pay.payMethod != null && !pay.payMethod.isEmpty() ? "\n支付方式：" + pay.payMethod : "")
                            + (pay.payRemark != null && !pay.payRemark.isEmpty() ? "\n备注：" + pay.payRemark : ""));
                } else {
                    addCardText("金额：" + pay.payAmount + " 状态：" + safe(pay.payStatus)
                            + (pay.payMethod != null && !pay.payMethod.isEmpty() ? "\n支付方式：" + pay.payMethod : "")
                            + (pay.payRemark != null && !pay.payRemark.isEmpty() ? "\n备注：" + pay.payRemark : ""));
                }
                if (detail.creator != null) {
                    addSectionTitle("收款方");
                    addCardText("" + detail.creator.displayName());
                    if (detail.creator.paymentQrCodePath != null && !detail.creator.paymentQrCodePath.isEmpty()) {
                        addSecondaryText("收款码：");
                        addRemoteImage(detail.creator.paymentQrCodePath);
                    }
                }
                if (pay.paymentProofPath != null && !pay.paymentProofPath.isEmpty()) {
                    addSectionTitle("付款凭证");
                    addRemoteImage(pay.paymentProofPath);
                }
                if (STATUS_UNPAID.equals(pay.payStatus)) {
                    addButton("选择支付方式并完成付款", v -> showPaymentMethodDialog(pay.payID));
                }
                addSecondaryButton("返回我的付款", v -> loadPays());
            }
    ...
    private void loadReports() {
        ...
        ApiClient.get().myReports().enqueue(new Callback<ApiResponse<List<Report>>>() {
            ...
            public void onResponse(Call<ApiResponse<List<Report>>> call, Response<ApiResponse<List<Report>>> response) {
                ...
                if (body.data == null || body.data.isEmpty()) {
                    addSecondaryText("暂无举报记录");
                    return;
                }
                for (Report report : body.data) {
                    addCardText("举报#" + report.reportID + " 活动#" + report.accusedActivityID
                            + "\n原因：" + safe(report.reportReason)
                            + "\n状态：" + safe(report.reportProcessStatus));
                    if (STATUS_PENDING_REVIEW.equals(report.reportProcessStatus)) {
                        addSecondaryButton("撤销举报 #" + report.reportID, v -> confirmCancelReport(report.reportID));
                    }
                }
            }
    ...
    private void renderProfile(OrdinaryUser user) {
        ...
        addCardText("用户ID：" + user.userID
                + "\n用户名：" + user.displayName()
                + "\n手机号：" + safe(user.phoneNumber)
                + "\n邮箱：" + safe(user.userMailbox)
                + "\n真实姓名：" + safe(user.realName)
                + "\n性别：" + safe(user.gender)
                + "\n生日：" + safe(user.birthday)
                + "\n邀请码：" + safe(user.inviteCode));
        if (user.totalIncome != null || user.totalExpense != null) {
            addSectionTitle("收支统计");
            addCardText("总收入：" + safe(user.totalIncome) + "\n总支出：" + safe(user.totalExpense));
        }
        addSectionTitle("收款码");
        if (user.paymentQrCodePath != null && !user.paymentQrCodePath.isEmpty()) {
            addRemoteImage(user.paymentQrCodePath);
        } else {
            addSecondaryText("尚未上传收款码");
        }
        addButton("编辑个人信息", v -> showEditProfileDialog(user));
        addSecondaryButton("修改密码", v -> showChangePasswordDialog());
        addSecondaryButton("上传/更新收款码", v -> openImagePicker(REQUEST_PAYMENT_QR_CODE));
        addDangerButton("注销账号", v -> confirmDeleteCurrentUser());
    }
            ...
            public void onResponse(Call<ApiResponse<List<OrdinaryUser>>> call, Response<ApiResponse<List<OrdinaryUser>>> response) {
                ...
                if (body.data == null || body.data.isEmpty()) {
                    ...
                    for (OrdinaryUser user : body.data) {
                        ...
                        matchedCount++;
                        addCardText("#" + user.userID + " " + user.displayName()
                                + "\n手机号：" + safe(user.phoneNumber)
                                + "  邮箱：" + safe(user.userMailbox));
                        addDangerButton("删除用户 #" + user.userID, v -> confirmAdminDeleteUser(user.userID));
                    }
                    if (matchedCount == 0) {
                        addSecondaryText("没有匹配的用户");
                    }
                }
                addSecondaryButton("返回管理员面板", v -> loadAdminPanel());
            }
    ...
    private void addAdminUserSearchControls() {
        addCardText("搜索关键词：" + safe(adminUserKeyword));
        addSecondaryButton("搜索用户", v -> showAdminUserSearchDialog());
        if (adminUserKeyword != null && !adminUserKeyword.isEmpty()) {
            addSecondaryButton("清除搜索", v -> {
    ...
    private void loadAdminActivities() {
        ...
        ApiClient.get().activities(null, null, null).enqueue(new Callback<ApiResponse<List<GroupActivity>>>() {
            ...
            public void onResponse(Call<ApiResponse<List<GroupActivity>>> call, Response<ApiResponse<List<GroupActivity>>> response) {
                ...
                if (body.data == null || body.data.isEmpty()) {
                    addSecondaryText("暂无项目");
                } else {
                    for (GroupActivity activity : body.data) {
                        addCardText(activity.titleLine()
                ...
                if (body.data == null || body.data.isEmpty()) {
                    ...
                    for (Report report : body.data) {
                        ...
                        if (STATUS_PENDING_REVIEW.equals(report.reportProcessStatus)) {
                            ...
                            addSecondaryButton("驳回举报 #" + report.reportID,
    ...
    private void setLoading() {
        ...
        loadingContainer.setPadding(0, 60, 0, 60);
    ...
    private void addSectionTitle(String text) {
        ...
        view.setPadding(4, 16, 4, 12);
        contentLayout.addView(view);
    }

    private void addCardText(String text) {
